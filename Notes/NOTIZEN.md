...
| Lorenzo-Brief | Ermittlung_Familia_Antwort | ✅ Passage drin — aber EINSTIEG FEHLT (s.u.) |
...
2. **Familia-Einstieg:** `familia_kontaktiert` wird nirgends gesetzt → Lorenzo-Brief kann nie feuern. Andockpunkt: `Stadt_Botenstand` bekommt ab Spur `nachtfuhre` die Option „Wort an Lorenzo schicken" (setzt `familia_kontaktiert = true` + `familia_kontakt_tag = $world.day`). Brief kommt +5 Tage via Wenzel. Inhaltlich: bestätigt den Kanal von außen („Schau in dein eigenes Haus"), liefert aber keinen Kopf.
